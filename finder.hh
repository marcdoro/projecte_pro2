...
class Finder {
    ...
    public:
        ...
        void update(T *t) {remove(t);add(t);}

        void remove(T *t) {
            auto existeix = object_chunks_.find(t);
            if (existeix == object_chunks_.end()) return; 
            ...
            for (const auto& chunk_id : chunks_with_t) {
                ...
                if (it_chunk != chunks_.end()) {
                    ...
                    if (it_chunk->second.empty()) chunks_.erase(it_chunk); // Si s'ha quedat buit eliminem tot el relacionat al chunk
                }
            }
                ...
                for (int columna = top_r; columna < bottom_r + 1; columna++) {
                    ...
                    if (it_chunk != chunks_.end()) {
                        for (const auto& e : it_chunk->second) {
                            if (collision(e->get_rect(), qrect)) {result.insert(e);}
                        }
                    }
                }
            }
